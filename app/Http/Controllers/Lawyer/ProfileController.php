...
class ProfileController extends Controller
{
    ...
    public function updatePassword(Request $request)
    {
        ...
        if (!Hash::check($request->current_password, $passwordHash)) {
            return redirect()->route('lawyer.profile.index')->with('error_message', 'Неверный текущий пароль');
        }
        if ($request->password != $request->password_confirmation) {
            return redirect()->route('lawyer.profile.index')->with('error_message', 'Пароли разнятся');
        }
        User::where('id', Auth::id())->update(
            [
