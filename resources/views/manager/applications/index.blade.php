...
@section('content')
    ...
    <div class="table-responsive">
        <table class="table table-striped">
            <a href="{{route('manager.applications.create')}}" class="btn btn-outline-primary waves-effect">Новая
                заявка</a>
            <thead>
            <div class="row mb-2">
                <div class="col-md-12 col-lg-12">
                    <form role="form" method="get" action="{{route('manager.applications.index')}}">
                        <br>
                        <div class="row">
            ...
            @foreach($applications as $application)
                <tr>
                    ...
                    <td>{{optional($application->manager)->name}}</td>
                    <td><a href="{{route('manager.lawyers.edit',[$application->lawyer_id])}}">{{optional($application->lawyer)->name}}</a></td>
                    <td>{{$application->description}}</td>
                    <td><a href="{{route('manager.applications.edit',$application->id)}}"
                           class="btn btn-outline-dark btn-sm waves-effect">Редактировать</a></td>
                    <td><a href="{{route('manager.applications.destroy',$application->id)}}"
