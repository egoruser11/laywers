...
@section('content')
    ...
    @endif
    <a href="{{route('manager.courts.create')}}" class="btn btn-outline-primary waves-effect">Новый
        суд</a>
        ...
        <table class="table table-striped">
            ...
            @foreach($courts as $court)
                <tr>
                    <td>{{$court->id}}</td>
                    <td>{{$court->name}}</td>
                    <td><a href="{{route('manager.courts.edit',$court->id)}}"
                           class="btn btn-outline-dark btn-sm waves-effect">Редактировать</a></td>
                    <td><a href="{{route('manager.courts.destroy',$court->id)}}"
